...
class SearchResponse(BaseModel):
    ...
    oneof_schema_2_validator: Optional[ChatResponse] = None
    actual_instance: Optional[Union[ChatResponse, SourceDocuments]] = None
    one_of_schemas: Set[str] = {"ChatResponse", "SourceDocuments"}
    ...
    def actual_instance_must_validate_oneof(cls, v):
        ...
        error_messages = []
        match = 0
        # validate data type: SourceDocuments
        if not isinstance(v, SourceDocuments):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SourceDocuments`")
        else:
            match += 1
        # validate data type: ChatResponse
        if not isinstance(v, ChatResponse):
            error_messages.append(f"Error! Input type `{type(v)}` is not `ChatResponse`")
        else:
            match += 1
        if match > 1:
            # more than 1 match
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # deserialize data into SourceDocuments
        try:
            instance.actual_instance = SourceDocuments.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
        # deserialize data into ChatResponse
        try:
            instance.actual_instance = ChatResponse.from_json(json_str)
            match += 1
        except (ValidationError, ValueError) as e:
            error_messages.append(str(e))
    ...
    def to_dict(self) -> Optional[Union[Dict[str, Any], ChatResponse, SourceDocuments]]:
