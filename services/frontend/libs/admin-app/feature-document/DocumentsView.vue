...
watch(allDocuments, (newDocs, oldDocs) => {
  if (oldDocs && newDocs && newDocs.some((doc: DocumentModel) => doc.status === "UPLOADING" || doc.status === "PROCESSING")) {
    setTimeout(documentStore.loadDocuments, 3000);
  }
}, { deep: true });
...
<template>
  <div data-testid="document-view" class="md:container md:mx-auto h-full p-4 flex gap-4">
        ...
        <div class="flex gap-4 items-center p-4 pt-6">
          <div class="flex flex-col flex-1">
            ...
            <div class="text-sm opacity-50">
              {{ t("documents.uploadDocumentsInfo") }}
            </div>
          </div>
        </div>
