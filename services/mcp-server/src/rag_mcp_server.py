...
class RagMcpServer:
    ...
    def _register_tools(self):
        ...
        async def chat_simple(session_id: str, message: str) -> str:
            chat_request = ChatRequest(message=message)
            response = await self._handle_chat(session_id, chat_request)
            return response.answer

        chat_simple.__doc__ = self._settings.chat_simple_description

        async def chat_with_history(
            session_id: str, message: str, history: list[dict[str, str]] = None
        ) -> dict[str, Any]:
        ...
        chat_with_history.__doc__ = self._settings.chat_with_history_description

        # Simple tools for basic agent usage
        self._server.add_tool(self._server.tool(chat_simple))
        self._server.add_tool(self._server.tool(chat_with_history))
